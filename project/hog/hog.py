...
def pigs_on_prime(player_score, opponent_score):
    ...
    if not is_prime(player_score):
        ...
        ret = 1
        while not is_prime(player_score+ret):
            ret += 1
        return ret
    # END PROBLEM 4
...
def make_averaged(original_function, total_samples=1000):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    def make_average_now(*args):
        tmp = 0
        i = 0
        while i < total_samples:
            tmp += original_function(*args)
            i += 1
        return tmp/total_samples
    return make_average_now
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    best_choice = 0
    while x <= 10:
        make_max_average = make_averaged(roll_dice, total_samples)
        tmp = make_max_average(x, dice)
        if tmp > curr_max:
            curr_max = tmp
            best_choice = x
        x += 1
...
def oink_points_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    tmp = oink_points(score, opponent_score)
    if tmp >= threshold:
        return 0
    else:
        return num_rolls
...
def pigs_on_prime_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    tmp = oink_points(score, opponent_score)
    if (pigs_on_prime(tmp+score, opponent_score)+tmp) >= threshold or pigs_on_prime(tmp+score, opponent_score) > 0:
        return 0
    else:
        return num_rolls
